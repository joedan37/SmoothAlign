...
class PCMEPPModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        if opt.criterion.name in ('smooth_intermediate', 'pcmepp_smooth') and opt.train.get('torch_compile'):
            raise ValueError(
                'intermediate prior criteria use forward hooks and must run with '
                'train.torch_compile disabled')
        ...
        self._prior_hook_cache = {}
        self._prior_hook_handles = []
        if self.opt.criterion.name in ('smooth_intermediate', 'pcmepp_smooth'):
            self._configure_intermediate_prior_hooks()
        ...
        if self.swa_enabled:
            ...
            if self.opt.criterion.name in ('smooth_intermediate', 'pcmepp_smooth'):
                self._configure_intermediate_prior_hooks(
                    self.swa_img_enc.module, self.swa_txt_enc.module)

        self.save_hyperparameters()

    def _configure_intermediate_prior_hooks(self, img_enc=None, txt_enc=None):
        """Capture the same intermediate CLIP block in both encoders.

        ``prior_layer`` is one-based to match the paper notation. The captured
        token features are used only to construct the detached smoothing prior;
        the contrastive logits still use the normal final-layer embeddings.
        """
        if self.opt.model.precomp_enc_type != 'backbone':
            raise ValueError(
                'smooth_intermediate requires end-to-end backbone features')

        img_enc = img_enc or self.img_enc
        txt_enc = txt_enc or self.txt_enc
        prior_layer = int(self.opt.criterion.get('prior_layer', 9))
        image_blocks = img_enc.backbone.transformer.resblocks
        text_blocks = txt_enc.backbone.transformer.resblocks
        if not (1 <= prior_layer <= len(image_blocks)):
            raise ValueError(
                f'Image prior_layer={prior_layer} is outside '
                f'[1, {len(image_blocks)}]')
        if not (1 <= prior_layer <= len(text_blocks)):
            raise ValueError(
                f'Text prior_layer={prior_layer} is outside '
                f'[1, {len(text_blocks)}]')

        layer_index = prior_layer - 1
        self._prior_hook_handles.append(
            image_blocks[layer_index].register_forward_hook(
                self._capture_image_prior_tokens))
        self._prior_hook_handles.append(
            text_blocks[layer_index].register_forward_hook(
                self._capture_text_prior_tokens))
        print(
            f'Smooth intermediate prior uses CLIP layer {prior_layer} '
            '(one-based) for both image and text encoders')

    def _capture_image_prior_tokens(self, module, inputs, output):
        self._prior_hook_cache['image'] = output

    def _capture_text_prior_tokens(self, module, inputs, output):
        self._prior_hook_cache['text'] = output

    @torch.no_grad()
    def _pool_intermediate_prior(self, img_enc, txt_enc, caption_lengths):
        """Project and pool captured LND tokens into paired prior features."""
        if 'image' not in self._prior_hook_cache or 'text' not in self._prior_hook_cache:
            raise RuntimeError(
                'Intermediate prior hooks did not capture both modalities')

        image_tokens = self._prior_hook_cache.pop('image').detach()
        text_tokens = self._prior_hook_cache.pop('text').detach()
        if isinstance(image_tokens, dict):
            image_tokens = image_tokens['mean']
        if isinstance(text_tokens, dict):
            text_tokens = text_tokens['mean']

        # CLIP residual blocks expose sequence-first LND tensors.
        ...
        img_model = getattr(img_enc, 'module', img_enc)
        txt_model = getattr(txt_enc, 'module', txt_enc)
        image_prior = img_model.image_encoder(image_tokens, image_lengths)

        # Match the normal CLIP text path after the selected transformer block.
        text_tokens = txt_model.backbone.ln_final(text_tokens)
        text_tokens = txt_model.linear(text_tokens)
        caption_lengths = caption_lengths.long().to(text_tokens.device)
        if txt_model.gpool is None:
            text_prior = text_tokens[
                torch.arange(len(caption_lengths), device=text_tokens.device),
                caption_lengths - 1, :]
        else:
            text_prior, _ = txt_model.gpool(text_tokens, caption_lengths)
        if not txt_model.no_txtnorm:
            text_prior = F.normalize(text_prior, dim=-1)

        return image_prior.detach(), text_prior.detach()
    ...
    def forward_step(self, batch, mixup_fn=None, use_swa=False):
        use_intermediate_prior = (
            self.opt.criterion.name in ('smooth_intermediate', 'pcmepp_smooth'))
        if use_intermediate_prior:
            self._prior_hook_cache.clear()
        ...
        caption_lengths = torch.as_tensor(
            lengths, dtype=torch.long, device=captions.device)
        cap_emb = txt_enc(captions, caption_lengths)
        if use_intermediate_prior:
            image_prior, caption_prior = self._pool_intermediate_prior(
                img_enc, txt_enc, caption_lengths)
            img_emb['prior'] = image_prior
            cap_emb['prior'] = caption_prior
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.opt.train.get('dist_train'):
            ...
            if self.opt.train.get('all_gather_infonce'):
                ...
                prior_matrix = None
                if self.opt.criterion.name in ('smooth', 'smooth_intermediate'):
                    if self.opt.criterion.name == 'smooth_intermediate':
                        img_prior_gat = grad_all_gather(img_emb['prior'])
                        cap_prior_gat = grad_all_gather(cap_emb['prior'])
                        img_prior_all = torch.cat(
                            [img_emb['prior']] + img_prior_gat[:rank]
                            + img_prior_gat[rank + 1:])
                        cap_prior_all = torch.cat(
                            [cap_emb['prior']] + cap_prior_gat[:rank]
                            + cap_prior_gat[rank + 1:])
                    else:
                        img_prior_all = img_emb_all
                        cap_prior_all = cap_emb_all
                    prior_matrix_all = self.criterion.build_prior(
                        img_prior_all, cap_prior_all)
                    prior_matrix = prior_matrix_all[:len(img_emb['mean'])]
                ...
                prior_matrix_i2t = None
                prior_matrix_t2i = None
                if self.opt.criterion.name == 'pcmepp_smooth':
                    img_prior_gat = grad_all_gather(img_emb['prior'])
                    cap_prior_gat = grad_all_gather(cap_emb['prior'])
                    img_prior_all = torch.cat(
                        [img_emb['prior']] + img_prior_gat[:rank]
                    ...
                    prior_matrix_all = self.criterion.build_prior(
                        img_prior_all, cap_prior_all)
                    local_size = len(img_emb['mean'])
                    prior_matrix_i2t = prior_matrix_all[:local_size]
                    prior_matrix_t2i = prior_matrix_all.T[:local_size]

...
def evaluate_ret(model, tasks, val_loader, global_step):
    ...
    for task in subtasks:
        ...
        if task!="tvas" and task!="tva":
            condition_feats_key = f'condition_feats_{task}'
            if isinstance(store_dict[condition_feats_key], list):
                store_dict[condition_feats_key] = torch.cat(store_dict[condition_feats_key],dim=0)
        itm_rerank_num = model.config.itm_rerank_num

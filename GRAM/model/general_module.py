...
class MMGeneralModule(nn.Module):
    ...
    def load_videoswin_model(self):
        ...
        videoswin_weight = torch.load('./weights/encoders/videoswin_base_k600_22k.pth',map_location='cpu')['state_dict']
    ...
    def load_beats_model(self):
        ...
        checkpoint = torch.load('./weights/encoders/beats/BEATs_iter3_plus_AS2M.pt')
    ...
    def load_ast_model(self):
        ...
        ast_weight = torch.load('./weights/encoders/audioset_10_10_0.4593.pth',map_location='cpu')
        audio_weight = {}
    ...
    def load_clip_model(self):
        ...
        if self.config.vision_encoder_type.startswith('evaclip'):
            ...
            if  self.config.vision_encoder_type == 'evaclip02_base':
                ...
                pretrained = "./weights/encoders/clip/EVA02_CLIP_B_psz16_s8B.pt" 
                self.vision_dim = 768
                
            elif self.config.vision_encoder_type == 'evaclip02_base_self':
                model_name = "EVA02-CLIP-B-16" 
                pretrained = "./weights/encoders/clip/EVA02_B_psz14to16.pt"
                self.vision_dim = 768

            elif self.config.vision_encoder_type == 'evaclip02_large':
                model_name = "EVA02-CLIP-L-14" 
                pretrained = "./weights/encoders/clip/EVA02_CLIP_L_psz14_s4B.pt"
                self.vision_dim = 1024

            elif self.config.vision_encoder_type == 'evaclip02_bige':
                model_name = "EVA02-CLIP-bigE-14-plus" 
                pretrained = "./weights/encoders/clip/EVA02_CLIP_E_psz14_plus_s9B.pt" 
                self.vision_dim = 1792

            elif self.config.vision_encoder_type == 'evaclip01_giant':
                model_name = "EVA01-CLIP-g-14" 
                pretrained = "./weights/encoders/clip/EVA01_CLIP_g_14_psz14_s11B.pt"
            ...
            if  self.config.vision_encoder_type == 'clip_vit_base_16':
                clip_weight = torch.jit.load('./weights/encoders/clip/ViT-B-16.pt', map_location='cpu')
                self.vision_dim = 768
            elif self.config.vision_encoder_type == 'clip_vit_large_14_336px':
                clip_weight = torch.jit.load('./weights/encoders/clip/ViT-L-14-336px.pt', map_location='cpu')
                self.vision_dim = 1024
            elif self.config.vision_encoder_type == 'clip_vit_base_32':
                clip_weight = torch.jit.load('./weights/encoders/clip/ViT-B-32.pt', map_location='cpu')
                self.vision_dim = 768
    ...
    def forward_vision_encoder(self, vision_pixels, return_intermediate_features=False, intermediate_layer=None):   ### b,n,3,h,w
        ...
        if self.config.vision_encoder_type.startswith('clip') or self.config.vision_encoder_type.startswith('evaclip'):
            ...
            if return_intermediate_features and self.config.vision_encoder_type.startswith('evaclip'):
                vision_output = self.vision_encoder.visual(
                    vision_pixels.reshape(b*n,3,h,w),
                    return_all_features=True,
                    return_intermediate_features=True,
                    intermediate_layer=intermediate_layer,
                )
                vision_output, intermediate_output = vision_output
                intermediate_output = intermediate_output.reshape(b,-1,*intermediate_output.shape[-2:])
            else:
        ...
        if return_intermediate_features and intermediate_output is not None:
            return vision_output, intermediate_output
    ...
    def forward_audio_encoder(self, audio_spectrograms, return_intermediate_features=False, intermediate_layer=None):        
        intermediate_output = None
        if self.config.audio_encoder_type.startswith('ast'): 
            ...
            if return_intermediate_features:
                ...
                audio_output, intermediate_output = audio_output
                intermediate_output = intermediate_output.reshape(b,n,-1,intermediate_output.shape[-1])
            else:
        ...
        if return_intermediate_features and intermediate_output is not None:
            return audio_output, intermediate_output
        return audio_output
    ...
    def load_swin_model(self):
        ...
        if self.config.vision_encoder_type.startswith('swin_base_22k_224'):
            swin_config = get_config('./weights/encoders/swin/swin_base_patch4_window7_224_22k.yaml')
            swin_weight = torch.load('./weights/encoders/swin/swin_base_patch4_window7_224_22k.pth', map_location='cpu')['model']
            self.vision_dim=1024
        elif self.config.vision_encoder_type.startswith('swin_large_22k_224'):
            swin_config = get_config('./weights/encoders/swin/swin_large_patch4_window7_224_22k.yaml')
            swin_weight = torch.load('./weights/encoders/swin/swin_large_patch4_window7_224_22k.pth', map_location='cpu')['model']
